...
- [x] **Step 1: Add PairingStore interface**

让 `MemoryPairingStore` 实现 `PairingStore`。

- [x] **Step 2: Add JsonFilePairingStore**
...
- [x] **Step 1: Add SessionTokenStore interface**

让 `MemorySessionTokenStore` 实现接口，并增加 `findTokenForBinding()`。

- [x] **Step 2: Add JsonFileSessionTokenStore**
...
- [x] **Step 1: Select stores from config**

`dataDir === null` 用内存 store；否则用 JSON file store。

- [x] **Step 2: Recover approved request auth**
